...
function Connection(socket) {
  ...
  // Maps from collection -> doc ID -> presence ID -> presence objects
...
Connection.prototype._setState = function(newState, reason) {
  ...
  // Emit the event to all LocalPresences
  for (var collection in this._localPresences) {
    var docs = this._localPresences[collection];
    for (var id in docs) {
      var presences = docs[id];
      for (var presenceId in presences) {
        presences[presenceId]._onConnectionStateChanged();
      }
    }
  }
...
Connection.prototype.getPresence = function(collection, id, presenceId) {
  return util.digOrCreate(this._localPresences, collection, id, presenceId, function() {
    return new LocalPresence(this, presenceId, collection, id);
  }.bind(this));
};

Connection.prototype.getPresences = function(collection, id) {
  var docPresences = util.dig(this._localPresences, collection, id) || {};
  return Object.keys(docPresences).map(function(presenceId) {
    return docPresences[presenceId];
  });
};

Connection.prototype._subscribeToPresence = function(collection, id, callback) {
  this._presenceNoOp(collection, id, {requestPresence: true}, callback);
};

// TODO: Also clear remote presence from Doc
Connection.prototype._unsubscribeFromPresence = function(collection, id, callback) {
  this._destroyAllLocalPresence(collection, id, function(error) {
    if (error) {
      if (callback) return callback(error);
      return this.emit('error', error);
    }

    this._presenceNoOp(collection, id, {unsubscribe: true}, callback);
  }.bind(this));
};

// A no-op presence message used to send messages to the agent to handle things like
// subscription, or requesting ops from other clients
Connection.prototype._presenceNoOp = function(collection, id, options, callback) {
  var presence = this.getPresence(collection, id, hat());
  presence.update(null, options, function(error) {
    if (error) {
      if (callback) return callback(error);
      return presence._doc.emit('error', error);
    }
    presence.destroy(callback);
  });
};

Connection.prototype._destroyAllLocalPresence = function(collection, id, callback) {
  var docPresences = util.dig(this._localPresences, collection, id) || {};
  var presenceIds = Object.keys(docPresences);

  async.each(presenceIds, function(presenceId, done) {
    var presence = docPresences[presenceId];
    presence.destroy(done);
  }, callback);
};

Connection.prototype._destroyLocalPresence = function(collection, id, presenceId) {
  util.digAndRemove(this._localPresences, collection, id, presenceId);
};

Connection.prototype._destroyRemotePresence = function(collection, id, presenceId) {
  util.digAndRemove(this._remotePresences, collection, id, presenceId);
};

Connection.prototype._getRemotePresence = function(collection, id, presenceId) {
  return util.digOrCreate(this._remotePresences, collection, id, presenceId, function() {
    return new RemotePresence(this, presenceId, collection, id);
  }.bind(this));
};

Connection.prototype._handlePresence = function(error, message) {
  var localPresence = util.dig(this._localPresences, message.c, message.d, message.id);
  ...
  if (presenceResponseRequested) {
    this._sendLocalPresences(message.c, message.d);
  }
};

Connection.prototype._sendLocalPresences = function(collection, id) {
  var docPresences = util.dig(this._localPresences, collection, id) || {};
  Object.keys(docPresences).forEach(function(id) {
    var localPresence = docPresences[id];
    localPresence.send();
  });
};
